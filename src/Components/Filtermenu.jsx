...
import useFilteredProducts from '../hooks/useFilteredProducts'; // Import the custom hook
import useFilterOptions from "../hooks/useFilterOptions";
import { useLocation } from "react-router-dom";

const Filtermenu = () => {
  const location = useLocation();
  const { tyreWidthOptions, tyreProfileOptions, rimSizeOptions } = useFilterOptions();
  ...
  const { handleFilterClick } = useFilteredProducts({
    selectedTyreWidth,
    selectedTyreProfile,
    selectedRimSize,
    selectedTubeType,
  ...
  useEffect(() => {
    ...
    // Automatically trigger filter click (fetch products) when page loads
    handleFilterClick();
  }, [location]); // Runs when the component is mounted or URL changes
  ...
  return (
    <div className="bg-slate-300 py-6 px-4 rounded-lg">
      <div className="bg-[#333333] px-6 py-6 rounded-lg flex justify-between">
        <div style={{ display: "flex", columnGap: "1em" }}>
          ...
          />
      <Button type="primary" className="bg-[#0055AA]" onClick={handleFilterClick}>
        Filter
      </Button>
        </div>
